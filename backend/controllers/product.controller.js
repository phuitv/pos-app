...
exports.createProduct = async (req, res, next) => {
    try {
        const product = await Product.create(req.body); //req.body chứa dữ liệu mà client (frontend) gửi lên khi tạo sp mới; Product.create(req.body): dùng Mongoose để tạo 1 bản ghi mới trong collection 'products'
        res.status(201).json({
            success: true,
            data: product
        });
...
exports.getProducts = async (req, res, next) => {
    try {
        const products = await Product.find();  //dùng Mongoose để lấy tất cả bản ghi
